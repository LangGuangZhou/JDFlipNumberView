...
- (NSArray<UIImage *> *)textToHalfImages:(NSString *)text {
    ...
    topLayer.frame = CGRectMake(0, 0, frame.size.width, (frame.size.height - 2)/2.0);
    topLayer.backgroundColor = UIColor.blueColor.CGColor;

    CALayer *bottomLayer = [CALayer new];
    [view.layer addSublayer:bottomLayer];
    bottomLayer.frame = CGRectMake(0, (frame.size.height + 2)/2.0, frame.size.width, (frame.size.height - 2)/2.0);
